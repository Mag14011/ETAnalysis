...
class PDBProcessor:
    ...
    def measure_avg_dist(self, pdb_file: str,
                        sequence_file: Optional[str] = None,
                        topology: Optional[str] = None) -> List[Dict]:
        """Calculate distances between consecutive heme pairs following a single path"""
        atoms_dict = self.read_pdb_atoms(pdb_file)
        ...
        if sequence_file:
            ...
            sequence = self.detect_sequence(atoms_dict, topology)
            if topology == 'linear':
                # For linear topology, use the sequence directly
                path = sequence
            else:
                # For branched topology, use the longest path
                paths = sequence if isinstance(sequence, list) else [sequence]
                path = max(paths, key=len)

            # Create pairs from consecutive residues in the path
            residue_pairs = list(zip(path[:-1], path[1:]))

        distances = []
        # Only measure distances between consecutive pairs in our chosen path
        for res1, res2 in residue_pairs:
            if res1 in atoms_dict and res2 in atoms_dict:
    ...
    def measure_subunit_length(self, pdb_file: str,
        ...
        For branched structures, uses the longest path through the structure.
        """
        atoms_dict = self.read_pdb_atoms(pdb_file)
        ...
        if sequence_file:
            ...
            if topology == 'linear':
                ...
                last_res = sequence[-1]
            else:  # branched topology
                sequences = self.detect_branched_sequence(atoms_dict)
                if not sequences:
                    raise ValueError("No valid sequences found in branched structure")
                
                # Find the longest path through the structure
                longest_path = max(sequences, key=len)
                first_res = longest_path[0]
                last_res = longest_path[-1]
    
        if 'FE' not in atoms_dict.get(first_res, {}) or 'FE' not in atoms_dict.get(last_res, {}):
            raise ValueError(f"Missing FE atoms in residues {first_res} or {last_res}")
        
        return np.linalg.norm(
            atoms_dict[first_res]['FE'] - atoms_dict[last_res]['FE']
        )
